...
#filename_queue = tf.train.string_input_producer(tf.train.match_filenames_once("./output/training_images/*.tfrecords"))
...
num_correct=tf.cast(tf.equal(tf.argmax(train_prediction,1),train_labels),"float")
...
ckpt = tf.train.get_checkpoint_state(os.path.dirname('./my-model'))
if ckpt and ckpt.model_checkpoint_path:
  saver.restore(sess, ckpt.model_checkpoint_path)

training_steps = 1
for step in range(training_steps):
  p, n, a = sess.run([train_prediction, num_correct, accuracy])
  print(p)
  print(n)
  print(a)
